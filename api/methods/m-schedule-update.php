  ...
  foreach($JSONSchedule as $Entry)
    ...
    foreach($speakers as $speaker)
      ...
      if($LinkResult && mysql_num_rows($LinkResult))
        ...
        $query = "UPDATE speakers SET name = '" . mysql_real_escape_string($speaker_name) . "'";

        if($speaker_company!='')
          {
          $query .= ",company = '" . mysql_real_escape_string($speaker_name) . "'";
          }
        if($speaker_url!='')
          {
          $query .= ",url = '" . mysql_real_escape_string($speaker_url) . "'";
          }
        if($speaker_twitter!='')
          {
          $query .= ",twitter = '" . mysql_real_escape_string($speaker_twitter) . "'";
          }
        if($speaker_image!='')
          {
          $query .= ",image = '" . mysql_real_escape_string($speaker_image) . "'";
          }
        if($speaker_slug!='')
          {
          $query .= ",slug = '" . mysql_real_escape_string($speaker_slug) . "'";
          }
        if($speaker_detail!='')
          {
          $query .= ",detail = '" . mysql_real_escape_string($speaker_detail) . "'";
          }
        if($speaker_title!='')
          {
          $query .= ",title = '" . mysql_real_escape_string($speaker_title) . "'";
          }
        if($speaker_abstract!='')
          {
          $query .= ",abstract = '" . mysql_real_escape_string($speaker_abstract) . "'";
          }
        if($speaker_bio!='')
          {
          $query .= ",bio = '" . mysql_real_escape_string($speaker_bio) . "'";
          }
        $query .= " WHERE speaker_id = " . $speaker_id;
        //echo $query . "<br />";
        mysql_query($query) or die('Query failed: ' . mysql_error());
